...
class ServiceNowConnector {
  ...
  sendRequest(callOptions, callback) {
    ...
    if (callOptions.query)
      uri = this.constructor.constructUri(
        callOptions.serviceNowTable,
        callOptions.query
      );
    else {
      uri = this.constructor.constructUri(callOptions.serviceNowTable);
    }
    /**
    ...
    request(requestOptions, (error, response, body) => {
      this.processRequestResults(
        error,
        response,
        body,
        (processedResults, processedError) =>
   ...
   */
  post(callback) {
    const postCallOptions = { ...this.options };
   ...
   */
  static constructUri(serviceNowTable, query = null) {
  ...
  processRequestResults(error, response, body, callback) {
    ...
    } else if (this.constructor.isHibernating(response)) {
